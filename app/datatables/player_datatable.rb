class PlayerDatatable < AjaxDatatablesRails::Base
  ...
  def data
    records.map do |record|
      ...
      [		
        "#{link_to record.name, profile_player_path(record)}", record.try(:country).name, record.batting_style.to_s.titleize, record.bowling_style, record.role.to_s.titleize, record.dob.to_date.strftime('%d/%m/%Y'), ops
      ]
    end
  end
